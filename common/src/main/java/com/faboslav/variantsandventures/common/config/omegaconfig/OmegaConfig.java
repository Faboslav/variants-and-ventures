package com.faboslav.variantsandventures.common.config.omegaconfig;
...
import com.faboslav.variantsandventures.common.platform.ConfigDirectory;
...
public final class OmegaConfig
{
		...
		try {
			...
			if (!configExists(config)) {
				...
				try {
					...
					VariantsAndVentures.getLogger().error(e.toString());
					VariantsAndVentures.getLogger().info(String.format("Encountered an error while reading %s config, falling back to default values.", config.getName()));
					VariantsAndVentures.getLogger().info(String.format("If this problem persists, delete the config file %s and try again.", config.getName() + "." + config.getExtension()));
				}
			}
	...
	public static <T extends Config> void writeConfig(Class<T> configClass, T instance) {
		...
		try {
			...
			VariantsAndVentures.getLogger().error(ioException.toString());
			VariantsAndVentures.getLogger().info(String.format("Write error, using default values for config %s.", configClass));
		}
	}
